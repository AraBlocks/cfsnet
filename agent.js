...
class CFSNetworkAgent {
  constructor(hostname, port) {
    ...
    if (/https?:/.test(hostname)) {
      ...
      hostname = parsed.hostname // eslint-disable-line prefer-destructuring
       ...
       */
      if (parsed.port && !port) {
        port = parsed.port // eslint-disable-line prefer-destructuring
      }
    } else {
  ...
  async request({
    method, uri, data, retry, headers, query
  }) {
