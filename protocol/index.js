...
class Protocol extends Duplex {
  ...
  send({operation, drive, buffer}, cb) {
    return new Promise((resolve, reject) => {
      ...
      const request = { operation, drive, buffer, nonce }
  ...
  async onhandshake(handshake, cb) {
    ...
    if (!nonce || 0 == nonce.length) {
      return cb(new ProtocolError(ProtocolError.BAD_HANDSHAKE_NONCE,
        "Invalid or missing nonce in handshake"))
    }

    if (!key || 0 == key.length) {
      return cb(new ProtocolError(ProtocolError.BAD_HANDSHAKE_KEY,
        "Invalid or missing key in handshake"))
    }

    if (false == this.isClient && ack !== false) {
      return cb(new ProtocolError(ProtocolError.BAD_HANDSHAKE_ACK,
        "Invalid or missing ack in handshake"))
    }

    if (false == this.isClient && false === this.verify(handshake)) {
      return cb(new ProtocolError(ProtocolError.BAD_HANDSHAKE_VERIFY,
  ...
  async onrequest(request, cb) {
    const self = this
    const { nonce, operation, drive, buffer } = request
    const {
      BAD_REQUEST_NONCE,
  ...
  async onoperation(operation, request, message, cb) {
    ...
    if (operation in ops) {
      if ('function' == typeof ops[operation]) {
        ...
        try { buffer = await ops[operation](state) }
        catch (err) { onoperationerror(err) }
      } else {
