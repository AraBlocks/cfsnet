 ...
 */
module.exports = { Open }
async function Open({request, operation, message, cfs}) {
  ...
  switch (flags[0]) {
    case 'r':
      if (flags.length > 2) { throw badFlagsError }
      else if (2 == flags.length) {
        if ('+' == flags[1]) { readonly = false }
        else { throw badFlagsError }
      }
      break
    ...
    case 'w':
      ...
      if (flags.length > 3) { throw badFlagsError }
      else if (2 == flags.length) {
        if ('+' == flags[1]) { void 0 }
        else if ('x' == flags[1]) { createIfNotExists = false }
        else { throw badFlagsError }
      } else if (3 == flags.length) {
        if ('x' == flags[1] && '+' == flags[2]) { void 0 }
        else { throw badFlagsError }
      }
      break
  ...
  if (false == readonly) {
    ...
    if (false == createIfNotExists) {
      ...
      try { await cfs.access(op.path) }
      catch (err) { await cfs.touch(op.path) }
    }
  ...
  try { fileDescriptor = await cfs.open(op.path) }
